...
exports.refresh = function(base, files, sync, err) {
    ...
    sh.exec('browser-sync --server "' + base + '" --files "' + files + '" --ghostMode ' + sync + '');
    if (err) { throw err; }
};
...
exports.lint = function(type, path, err) {
    ...
    case 'csslint':
        ...
        sh.echo('→ Validating scripts'.green);
        sh.exec('csslint ' + path + 'style.styl');
        if (err) { throw err; }
        break;
    }
};
...
exports.compile = function(type, input, output, err) {
    ...
    case 'uglifyjs':
        ...
        sh.exec('cat '+ input +' main.js | uglifyjs -o '+ output +'main.min.js');
        if (err) { throw err; }
        break;
    ...
    case 'stylus':
        ...
        sh.exec('stylus -c <' + input + 'style.styl> ' + output + 'style.css');
        if (err) { throw err; }
        break;
    }
};
